...
class MidiMixIterDataset(IterableDataset):
    ...
    def _build_dataset(self, root_dir, shuffle=True):
        df = []
        print(root_dir)
        audio_files = glob(f'{root_dir}/MIDI/**/{self.audio_filename}')
        for a_f in audio_files:
            ...
            df.append({'inst_names': inst_names["names"], 'audio_path': a_f, 'midi_path': midi_path})
        assert len(df) > 0
    ...
    def _parse_midi(self, path, instrument_dict):
        note_seqs = []

        for filename in instrument_dict:
        ...
        return note_seqs, instrument_dict
...
if __name__ == '__main__':
    ...
    dataset = MidiMixIterDataset(root_dir='C:/Users/ameyd/Documents/Assignments/ece324/mt3-pytorch/train_data')
